...
| Signed result durable; no execution attempted | Signed envelope available; the caller may BEGIN execution only while the approval is still admissible. Corrected (F7): "only before expiry under all existing runner checks" was false — expiry was sampled once and the preparation between that sample and the executor was unbounded. The invocation now carries one fixed deadline, re-checked before the nonce claim, before the durable intent, before the executor, before any credentialed connection, and after the executor's blocking locks. It is not atomicity: a check that passes can be followed by descheduling. | Signing/issuance evidence, not a database commit. Crash before delivery leaves delivery unknown. |
