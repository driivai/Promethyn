...
| `importorskip("psycopg")` | test_secret_canary_sweep::test_residual_a_psycopg_error_object_still_carries_the_password | **YES** — `psycopg` is installed by `pip install -e ".[dev]"`, which every CI job runs; the driver is also required by the migration runner it guards | **YES** | F8: pins the named residual — `str(exc)` is clean (so the diagnostics built from it are safe) while `vars(exc)` still carries the password through the driver's connection object | **yes** | **covered, and cannot silently skip** — the PROD-FIX-2 CI step asserts zero skips over this file, so an absent driver FAILS the build rather than passing quietly. `importorskip` is the right local shape (a clear message rather than an ImportError) but it is not what decides whether the test runs |
