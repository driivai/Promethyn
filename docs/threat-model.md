...
| 5 | Misconfiguration | every security flag and combination | requirement honoured or refused; dead-flag SPELLING check plus a behavioural proof per outcome-affecting field (G17); coherent combinations; hardened defaults | pin the sandbox image | `test_security_posture.py` | `require_digest_pin` and `require_ledger_anchor` off by default |
...
| `verifier_*` caps, `Limits` | Config, env | sandbox adapters | NaN/inf/negative refused at load. A bare `0` is refused too: unbounded is a supported posture but must be NAMED (`verifier_memory_mb=UNBOUNDED`), because removing a cap turns a `FAIL` into a `PASS` on identical candidate code (G21). `verifier_timeout_s`, `request_timeout_s` and `provider_max_response_bytes` have no unbounded spelling at all | enforced (§3, G21) |
...
| `require_ledger_anchor` | Config, env (OR of sources) | `Config.__post_init__`, then `build_tip_anchor_for` | no anchor, or a `file://` one → **refused at LOAD** ("cannot be honoured"), so such a `Config` cannot be constructed at all; the builder is a second site whose only reachable driver is `PROM_REQUIRE_LEDGER_ANCHOR` | enforced (§3, PIH-1) |
